                    ...
                    >
                        {(() => {
                            const DEFAULT_CATEGORIES = [
                                { name: 'School', desc: 'Syllabus and practice materials for all boards and classes.' },
                                { name: 'University', desc: 'Comprehensive guides for university exams and degree preparations.' },
                                { name: 'UPSC', desc: 'Mock tests, previous year papers, and study material for UPSC.' },
                                { name: 'CTET', desc: 'Central Teacher Eligibility Test preparation and practice modules.' },
                                { name: 'SSC', desc: 'Complete preparation for Staff Selection Commission exams.' },
                                { name: 'Banking', desc: 'Bank PO, Clerk, and specialist officer exam prep.' },
                                { name: 'Railway', desc: 'RRB NTPC, Group D, and other railway exams.' },
                                { name: 'State Govt Exams', desc: 'State public service commission and local government exams.' },
                                { name: 'Others', desc: 'Various other competitive and entrance examinations.' }
                            ];

                            return DEFAULT_CATEGORIES.map((defaultCat, idx) => {
                                // Try finding in DB
                                const dbCat = categories.find(c => c.name.toLowerCase() === defaultCat.name.toLowerCase());
                                // Only hide if explicitly marked inactive in DB
                                if (dbCat && !dbCat.is_active) return null;

                                const displayId = dbCat ? dbCat.id : `default-${idx}`;
                                const imgUrl = dbCat?.image_url;
                                const desc = dbCat?.description || defaultCat.desc;

                                return (
                                    <div key={displayId} className="snap-start flex-shrink-0 w-[280px] group cursor-pointer bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_rgb(0,0,0,0.08)] border border-gray-100 hover:border-indigo-100 transition-all duration-300 transform hover:-translate-y-2 overflow-hidden flex flex-col">
                                        <div className="h-44 bg-gray-100 relative overflow-hidden flex-shrink-0 flex items-center justify-center">
                                            {imgUrl ? (
                                                <img src={imgUrl} alt={defaultCat.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 ease-out" />
                                            ) : (
                                        ...
                                        <div className="p-6 flex-1 flex flex-col items-center text-center">
                                            <h3 className="text-xl font-extrabold text-gray-900 mb-3">{defaultCat.name}</h3>
