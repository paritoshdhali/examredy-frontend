        ...
        return (
            <div className="flex flex-col min-w-[160px] max-w-[200px] flex-shrink-0 relative">
                ...
                {isTableValid && hasInactive && (
                    <div className="px-3 py-3 border-b-2 border-indigo-600 bg-indigo-900/30 flex items-center justify-between cursor-pointer hover:bg-indigo-800/40 transition-all shadow-inner" onClick={handleCheckAll}>
                        <span className="text-xs font-black text-indigo-200 uppercase tracking-widest">Select All Hidden</span>
                        <input type="checkbox" checked={isAllSelected} onChange={() => { }} className="w-5 h-5 text-indigo-500 rounded border-indigo-500 bg-gray-900 focus:ring-0 cursor-pointer pointer-events-none shadow" />
                    </div>
                )}
                    ...
                    {items.length === 0
                        ...
                        : items.map(item => (
                            <div key={item.id} className="flex flex-col mb-1 border-b border-gray-800/50 pb-1">
                                ...
                                {isTableValid && (
                                    <div className="px-2 pt-2 border-t border-gray-800/60 mt-1">
                                        <div className="flex items-center justify-between cursor-pointer hover:bg-white/5 p-1 rounded transition-colors" onClick={(e) => handleCheck(e, item.id)}>
                                            <span className={`text-[10px] font-black uppercase tracking-wider ${item.is_active ? 'text-green-500' : 'text-orange-500 animate-pulse'}`}>{item.is_active ? 'LIVE' : 'HIDDEN'}</span>
                                            <input type="checkbox" checked={mapSet.has(item.id)} onChange={() => { }} className={`w-4 h-4 rounded focus:ring-0 cursor-pointer pointer-events-none ${item.is_active ? 'text-green-600 border-green-700 bg-green-900/20' : 'text-orange-500 border-orange-600 bg-orange-900/20'}`} />
                                        </div>
                                    </div>
                                )}
                            </div>
